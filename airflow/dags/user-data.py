...
COLS_TO_DROP = ['profile', 'color', 'who_can_share_contact_card', 
                    'team_id', '_corrupt_record']
COLS_BY_TOPIC = {
...
def extract_n_transform(spark: SparkSession):
    # Read Local Data and 
    try:
        all_users = spark.read.json(f'{PATH_TO_LOCAL_HOME}/{DATA_SOURCE_ROOT}/{USERS_DATA}') 
        all_users = all_users.drop(*COLS_TO_DROP)
        drop_expr = ' and '.join('(%s != "")' % col_name for col_name in all_users.columns)
...
with DAG(
    ...
    with TaskGroup("transform-and-upload-to-gcs") as transform_and_upload_to_gcs:
        ...
        cleanup = BashOperator(
            task_id="cleanup",
            bash_command=f'rm {PATH_TO_LOCAL_HOME}/temp.csv'
        )
    ...
    with TaskGroup("split-user-attributes") as split_user_attributes:
        ...
        cleanup = BashOperator(
            task_id="cleanup",
            bash_command=f'rm {PATH_TO_LOCAL_HOME}/temp*'
        )
    ...
    upload_raw_data_to_gcs >> transform_and_upload_to_gcs >> split_user_attributes >> create_external_tables
