...
channels = {
    "course-data-engineering":["_reactions", "_root_messages", "_thread_replies"], # parquet, csv, csv
    "course-ml-zoomcamps":["_reactions", "_root_messages", "_thread_replies"], # parquet, csv, csv
}
    ...
    create_root_messages_table = BigQueryUpsertTableOperator(
        ...
        table_resource = {            
            ...
            "externalDataConfiguration": {
                ...
                "sourceUris": [f"gs://{BUCKET}/clean/messages/{CHANNEL_NAME}*_rootmessages.parquet"]
            }        }
    )

    create_thread_replies_table = BigQueryUpsertTableOperator(
        task_id=f"thread_replies_table",
        dataset_id=BIGQUERY_DATASET,
        table_resource = {            
            "tableReference": {
                "projectId": PROJECT_ID,
            ...
            "externalDataConfiguration": {
                "sourceFormat": "PARQUET",
                "sourceUris": [f"gs://{BUCKET}/clean/messages/{CHANNEL_NAME}*_threadreplies.parquet"]
            }        }
    )

    create_reactions_table = BigQueryUpsertTableOperator(
        task_id=f"reactions_table",
        dataset_id=BIGQUERY_DATASET,
        table_resource = {            
            "tableReference": {
                "projectId": PROJECT_ID,
            ...
            "externalDataConfiguration": {
                "sourceFormat": "PARQUET",
                "sourceUris": [f"gs://{BUCKET}/clean/messages/{CHANNEL_NAME}*_reactions.parquet"]
            }        }
    )