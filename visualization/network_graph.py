from config import run_query, TABLE_ROOT_MESSAGES, TABLE_THREAD_REPLIES, EXTERNAL_TABLE_USERS_IDENTITY
import pandas as pd
...
def combine_with_users_data(df, df_users):
    ...
    df = df.rename(columns={'id':'target_id', 'name':'target_name', 'real_name': 'target_real_name'})
    df = pd.merge(df, df_users,  how='left', left_on=['source'], right_on = ['id'])
    df = df.rename(columns={'id':'source_id', 'name':'source_name','real_name': 'source_real_name'})
    df = df.drop(columns=['target_id', 'source_id', 'target_real_name', 'source_real_name', ])
...
def build_edges(df, network, node_labels, ids):
    ...
    for i, source_node in enumerate(node_labels):
        ...
        for tw in targets_weights:
            ...
            network.add_edge(i, target_node, value=weight, title=str(weight))  # weight 42
            # edges.append(edge)
...
def build_pyvis_network(df,name):
    ...
    network, node_labels, label_2_id_mapping,  = build_nodes(df=df,network=network)
